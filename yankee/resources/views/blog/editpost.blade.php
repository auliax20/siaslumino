...
<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="col-lg-12">
							<form role="form" method="POST" action="{{ asset('blog-manager/post/editpost/'.$post->id_post) }}" enctype="multipart/form-data">
                            	<input type="hidden" name="_token" value="{{ csrf_token() }}">
								<div class="form-group">
									<label>Judul</label>
                                                                        <input class="form-control" placeholder="Judul Postingan" name="title" value="{{ $post->title}}">
								</div>
								<div class="form-group">
									<label>Kategori</label>
                                                                        <select class="form-control" name="category">
                                                                            ...
                                                                            @if($data->id_category == $post->id_post)
                                                                            <option value="{{ $data->id_category }}" selected="selected">{{ $data->category }}</option>
                                                                            @else
                                ...
                                <div class="form-group">
									<label>Kontent Posting</label>
                                                                        <textarea placeholder="Ketikkan Post" name="post" id="post">{{ $post->post }}</textarea>
								</div>
								<div class="form-group">
